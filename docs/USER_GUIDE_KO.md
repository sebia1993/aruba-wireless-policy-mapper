...
웹 화면에서는 `접속 방식`을 먼저 선택합니다. SSH는 접속 포트 22, Telnet은 23이 자동 기본값으로 표시됩니다. 그 다음 `WLC IP`, 선택 항목인 `보고서 표시 이름`, `장비 ID`, `장비 PW`, 필요 시 Enable password를 입력합니다.

입력 오류는 `입력값을 확인하세요`와 함께 잘못된 항목을 표시합니다. 수집·파싱·보고서 생성 중 실패하면 결과 영역에 `오류 코드`, `실패 단계`, `권장 조치`가 먼저 표시되고, 내부 상태값은 `기술 세부 정보`를 펼쳤을 때만 보입니다. Python traceback이나 내부 파일 경로는 일반 결과 화면에 표시하지 않습니다.

## 5. GUI 입력 항목 설명

GUI의 기본 흐름은 `접속 정보 입력 → 분석 시작 → 결과 상태 확인 → HTML 보고서 확인`입니다.

- 왼쪽: WLC 접속 정보, 인증 정보, 결과 저장 위치 입력
- 결과 영역: SSID/Role 수치, `정상 완료·부분 완료·수집 실패`, 실패 명령 수, 영향 안내 표시
- `분석 시작` 옆 `실행 취소`: 현재 명령이 끝난 뒤 장비 세션을 닫고 안전하게 중단
...
| Timeout seconds | 명령 하나의 응답 대기 시간. 5~600초만 허용하며 기본값은 60초 |
...
| 상태 | 의미 | 사용자 조치 |
| --- | --- | --- |
| 정상 완료 | 필수/선택 수집 명령이 모두 성공 | HTML의 Unresolved와 세부 ACL 확인 |
| 부분 완료 | 보고서는 생성됐지만 선택 명령 하나 이상 실패 | 실패 명령, 영향 영역, 영향 Role/SSID 확인 후 해당 정보는 재수집 전 확정 판단 금지 |
| 수집 실패 | `show configuration effective` 같은 필수 데이터 없음 | 생성 결과를 정책 판단에 사용하지 말고 접속·권한·명령 문제 해결 후 재실행 |
| 사용자 취소 | 사용자가 실행 취소를 요청함 | 부분 raw/run.log는 참고만 하고 필요하면 처음부터 재실행 |

`실행 취소`는 실행 중인 스레드나 접속을 강제로 끊지 않습니다. 현재 장비 명령이 응답하거나 해당 명령의 Timeout에 도달하면 다음 명령을 실행하지 않고 세션을 종료합니다. 프로그램 창을 닫을 때도 실행 중 작업의 취소 여부를 묻고, 세션과 파일 정리가 끝날 때까지 창을 유지합니다.

명령별 Timeout과 별개로 전체 수집은 최대 60분입니다. 많은 Role/Alias에서 반복 Timeout이 발생해 60분 상한에 도달하면 가능한 결과까지만 부분 완료로 만들고 `duration_limit` 실패를 표시합니다.
...
- 첫 화면의 `결론 요약`에서 수집 상태, 수집 신뢰도, 실패 명령, 영향 Role/SSID, Unresolved, 동적 Role 참고, 사내 Role 대역 비교 상태와 권장 조치를 먼저 확인합니다.
- 우측 상태 배지는 `수집 정상`, `검토 필요`, `제한적 사용`, `재수집 필요` 중 하나로 표시됩니다. `제한적 사용`이나 `재수집 필요`이면 보고서를 확정 판단 자료로 사용하기 전에 실패 범위와 권장 조치를 확인해야 합니다.
- `동적 Role 참고`는 장애나 정책 위반 건수에 포함되지 않습니다. ClearPass/RADIUS에서 다른 Role이 배정될 가능성을 알리는 참고 정보입니다.
- `수집 신뢰도`는 수집 데이터의 완전성을 뜻하며 ACL 정책이 적절하다는 보증은 아닙니다.
- 실패 영향 범위는 실패한 `rights::<Role>` 또는 `netdestination::<Alias>`와 보고서에 수집된 ACL/SSID 관계를 기준으로 계산합니다. 수집이 일찍 중단되면 자동 식별되지 않은 대상이 있을 수 있습니다.
...
- 좁은 창이나 작은 화면에서는 ACL 표 안을 좌우로 스크롤해 오른쪽의 Service, Raw, Comment 열을 확인합니다. 페이지 전체가 아니라 ACL 표 영역만 좌우로 이동합니다.
- 선택한 Role 상단의 `Role 보고 설명`에는 상급자 보고용 설명이나 검토 의견을 입력할 수 있습니다.
- `선택 Role PNG 저장`은 현재 선택한 Role 한 개를 이미지로 저장합니다.

## 8. ACL 주석, Role 설명, 이미지 저장

ACL 행마다 Comment 입력란이 있습니다.

- 입력한 주석은 브라우저에 임시 저장됩니다.
- Role별 `Role 보고 설명`도 입력 즉시 같은 브라우저에 자동 저장됩니다.
- `주석 포함 HTML 저장` 버튼을 누르면 현재 ACL 주석과 Role 설명이 포함된 별도 HTML 파일을 저장할 수 있습니다.
- `선택 Role PNG 저장`을 누르면 현재 선택한 Role 한 개만 PNG로 저장됩니다.
- PNG에는 현재 화면에서 보이는 상태가 반영됩니다. `Raw 보기`, `Show other ACLs`, Alias 상세를 열거나 닫은 상태를 먼저 조정한 뒤 저장하십시오.
- PNG에서는 입력란과 버튼 대신 ACL 주석과 Role 설명이 읽기 쉬운 고정 텍스트로 표시됩니다.
- Role 내용이 매우 길면 파일명이 `_part_01_of_03.png` 같은 여러 PNG로 나뉩니다. 이때 브라우저에서 여러 파일 다운로드를 허용해야 할 수 있습니다.
- 생성된 PNG는 일반적으로 브라우저 기본 다운로드 폴더에 저장됩니다.
- PDF로 저장하려면 `PDF 저장/인쇄` 버튼을 누르고 브라우저 인쇄 화면에서 PDF 저장을 선택합니다.
- PDF는 전체 Role을 대상으로 하는 정적인 문서이므로 HTML처럼 클릭, 접기, 펼치기 기능은 유지되지 않습니다.
...
CLI에서 여러 WLC를 지정하면 장비 하나의 예상 밖 수집·파싱 오류가 다른 장비의 수집을 중단시키지 않습니다. 실패 장비는 raw와 보고서 상태에 남고 종료 코드 `1`을 반환합니다. 입력 오류는 `2`, 선택 명령 일부 실패는 `3`입니다. 출력 폴더나 보고서 저장 실패는 `WLC-RPT-001` 또는 `WLC-RPT-002`와 조치 안내를 표시하며 Python traceback을 일반 오류 안내로 사용하지 않습니다.
...
- 수집 상태: 정상 완료, 부분 완료, 수집 실패, 사용자 취소
- 부분 완료인 경우 실패 명령 수와 화면에 표시된 영향 영역
